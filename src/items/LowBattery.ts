...
export const itemMeta: ItemMeta = {
  icon: ItemIcon,
  getDescription: (item) =>
    `**On Skip**: Reduce the Understanding requirements of all chats in the Group Chat by **${(itemUtils.geometricSeries(item.level - 1, 0.95, 0.1, 1) * 100).toFixed(2)}%**.`,
...
export const itemBehavior: ItemBehavior = {
  ...
  {
    let reducedCount = 0;
    let reduceMultiplier = 1 - itemUtils.geometricSeries(params.item.level - 1, 0.95, 0.1, 1);
