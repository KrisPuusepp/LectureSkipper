...
export const itemMeta: ItemMeta = {
  icon: ItemIcon,
  getDescription: (item) =>
    `**On Skip**: Gain **${item.level * 15}%** extra Procrastination from the lecture.`,
...
export const itemBehavior: ItemBehavior = {
  beforeSkipLecture: (params) =>
  {
    ...
    params.lecture.procrastinationValue += Math.round(params.lecture.procrastinationValue * ((params.item.level * 15) / 100));
